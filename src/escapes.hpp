...
namespace Escapes
{
	...
	const int UNSATTURATED_OFFSET = 90;

	const int COLOR_OFFSET_BASE = 40;
	const int UNSATTURATED_OFFSET_BASE = 60;
    ...
    {
		return std::format("\033[3;{};30m{}\033[0m", COLOR_OFFSET_BASE + COLOR_MAP.at(c), s);
    }

    inline std::string ColoredBaseLight(const std::string &s, Color c)
    {
		return std::format("\033[3;{};30m{}\033[0m", COLOR_OFFSET_BASE + UNSATTURATED_OFFSET_BASE + COLOR_MAP.at(c), s);
    }
